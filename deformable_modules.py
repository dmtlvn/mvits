    ...
    def get_reference_points(spatial_shapes, valid_ratios, device):
        ...
        for lvl, shape in enumerate(spatial_shapes):
            ref_y, ref_x = torch.meshgrid(
                torch.linspace(0.5, shape[0] - 0.5, shape[0], dtype=torch.float32, device=device),
                torch.linspace(0.5, shape[1] - 0.5, shape[1], dtype=torch.float32, device=device),
            )
            ref_y = ref_y.reshape(-1)[None] / (valid_ratios[:, None, lvl, 1] * shape[0])
            ref_x = ref_x.reshape(-1)[None] / (valid_ratios[:, None, lvl, 0] * shape[1])
